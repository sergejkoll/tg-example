...
func (http *httpAdder) serveSumTwoNumbers(ctx *fiber.Ctx) (err error) {
	return http.serveMethod(ctx, "sumtwonumbers", http.sumTwoNumbers)
}
func (http *httpAdder) sumTwoNumbers(ctx *fiber.Ctx, requestBase baseJsonRPC) (responseBase *baseJsonRPC) {
	var err error
	var request requestAdderSumTwoNumbers
	...
	response.Result, err = http.svc.SumTwoNumbers(methodContext, request.First, request.Second)
	if err != nil {
...
func (http *httpAdder) serveBatch(ctx *fiber.Ctx) (err error) {
	...
	for _, request := range requests {
		...
		case "sumtwonumbers":
			...
			func(request baseJsonRPC) {
				responses.append(http.sumTwoNumbers(ctx, request))
				wg.Done()
			}(request)
		default:
...
func (http *httpAdder) serveMethod(ctx *fiber.Ctx, methodName string, methodHandler methodJsonRPC) (err error) {
	methodHTTP := ctx.Method()
	if methodHTTP != fiber.MethodPost {
		ctx.Response().SetStatusCode(fiber.StatusMethodNotAllowed)
		if _, err = ctx.WriteString("only POST method supported"); err != nil {
